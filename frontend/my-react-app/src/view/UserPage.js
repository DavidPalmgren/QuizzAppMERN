...
function ProfilePage() {
  ...
  const fetchUserByParam = async () => {
    try {
      const response = await fetch(`http://localhost:4040/api/user/${userName}`, {
        method: 'GET',
        headers: {
...
useEffect(() => {
  ...
  thresholds.forEach((threshold) => {
    if (
      ...
      !userData.achievements.some((achievement) => achievement.name === (threshold === 100 ? 'Newbie' : 'Intermediate'))
    ) {
  ...
  if (userName === username) {
    ...
    console.log('Username in URL does not match token username. Achievements will not be added.');
  }
...
const addAchievement = async (achievements) => {
  ...
  if (!username || !token) {
    console.log("USERNAME or TOKEN NOT IN LOCAL STORAGE. REDIRECT OR HANDLE THIS CASE.");
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Handle the error, for example, show a notification to the user
    }
  } catch (error) {
    console.error("Error adding achievement:", error);
    // Handle network errors, for example, show a notification to the user
  }
}
      ...
      <Container component="main" maxWidth="lg" style={{marginTop: "32px"}}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={6}>
            ...
            <CardContent style={{backgroundColor: "rgb(222 222 222)", borderRadius: "4px"}}>
            <Typography variant="body2" style={{ padding: "10px", color: "black" }}>Achievements</Typography>
  ...
  userData.achievements
    ...
    .map((entry, index) => (
<Card style={{ margin: "10px", borderRadius: "4px", border: '1px solid #1976D2', backgroundColor: "#f0f0f0" }} key={entry.timestamp}>
  <CardContent style={{ padding: "10px", display: "flex", flexDirection: "column" }}>
    <div style={{ display: "flex", justifyContent: "space-between" }}>
      <span style={{ backgroundColor: "#1976D2", color: "#fff", padding: "4px 8px", borderRadius: "4px" }}>
  ...
  <Card>
    <Typography variant="body2" style={{ padding: "10px" }}> None</Typography>
  </Card>
)}
            </CardContent>
  ...
  <CardContent>
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px', border: '1px solid #1976D2', padding: '8px', borderRadius: '10px' }}>
      <span style={{ fontWeight: "bold" }}>Total Cards Answered:</span>
      <Card style={{ backgroundColor: "#1976D2", color: "#fff", padding: "10px", borderRadius: "10px" }}>
        <span style={{ fontWeight: "bold" }}>{userData.totalCardsAnswered}</span>
      </Card>
    </div>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px', border: '1px solid #1976D2', padding: '8px', borderRadius: '10px' }}>
      <span style={{ fontWeight: "bold" }}>Total Correct Answers:</span>
      <Card style={{ backgroundColor: "#1976D2", color: "#fff", padding: "10px", borderRadius: "10px" }}>
        <span style={{ fontWeight: "bold" }}>{userData.totalCorrectAnswers}</span>
      </Card>
    </div>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px', border: '1px solid #1976D2', padding: '8px', borderRadius: '10px' }}>
      <span style={{fontWeight: "bold" }}>Percentage Correct:</span>
      <Card style={{ backgroundColor: "#1976D2", color: "#fff", padding: "10px", borderRadius: "10px" }}>
        <span style={{ fontWeight: "bold" }}>{((userData.totalCorrectAnswers / userData.totalCardsAnswered) * 100).toFixed(2) + '%'}</span>
      </Card>
    </div>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px', border: '1px solid #1976D2', padding: '8px', borderRadius: '10px' }}>
      <span style={{ fontWeight: "bold" }}>Total Time Studied:</span>
      <Card style={{ backgroundColor: "#1976D2", color: "#fff", padding: "10px", borderRadius: "10px" }}>
